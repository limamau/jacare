...
class AbstractRouter:
    ...
    def simulate(self, *norms: Tuple[Array, Array]):
    ...
    def simulate_routing_lv(
        self,
        routing_lv: int,
        hillslope_xd_norms: Tuple[Array, Array],
        hillslope_xs_norms: Tuple[Array, Array],
        hillslope_y_norms: Tuple[Array, Array],
        channel_xd_norms: Tuple[Array, Array],
        channel_xs_norms: Tuple[Array, Array],
        channel_y_norms: Tuple[Array, Array],
    ) -> None:
...
class HillslopeChannelRouter(AbstractRouter):
    ...
    def __init__(
        ...
        self.graph = integrate_distances_on_graph(
            read_graph(graph_file_path), attributes_dir, distance_name
        )
    ...
    @staticmethod
    def merge_channel_data(data: BasinData, up_dict: dict, seq_length: int) -> None:
...
def aggregate_descendents(
    ...
    for basin in graph.keys():
        new_graph[basin] = get_all_upstreams(basin, graph)

    del graph
    return new_graph


def aggregate_routing_lvs(routing_lvs_basins: List[List[int]]) -> List[List[int]]:
    ...
    with h5py.File(simulation_file_path, "r") as f:
        for basin_id in basin_ids:
            ...
            for up_id, up_l in items:
                # get upstreamflow from file
                dataset = f[str(up_id)]
                assert isinstance(dataset, Dataset), (
                    f"Expected Dataset type, but got {type(dataset)}"
                )
                up_q = dataset[:]
...
def integrate_distances_on_graph(
    ...
    for basin_id, upstream_ids in graph.items():
        ...
        for upstream_id in upstream_ids:
            ...
            distances = attributes_df.loc[upstream_id, distance_name] - l0
            if flag:
                new_graph[basin_id] = [(upstream_id, distances)]
                flag = False
            else:
                new_graph[basin_id].append((upstream_id, distances))

    return new_graph


def read_graph(file_path: str) -> Dict[int, List[int]]:
...
def read_routing_lvs(routing_lvs_dir: str, num_routing_lvs: int) -> List[List[int]]:
